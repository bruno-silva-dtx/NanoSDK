...
sudo perf stat -o ./results/quic/perf-loss-$loss-delay-$delay-n-$number_of_packets-s-$size_of_packets-i-$msg_interval-q-$qos.txt -a -r $runs -e power/energy-cores/,power/energy-gpu/,power/energy-pkg/  ./quic_client pub 'mqtt-tcp://emqx:14567' $qos topic $size_of_packets $number_of_packets  $msg_interval
